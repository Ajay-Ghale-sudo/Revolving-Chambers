...
{
    /// <summary>
    /// MonoBehaviour component for controlling the revolver chamber
    /// </summary>
    public class BossRevolverController : MonoBehaviour
    {
        /// <summary>
        /// Chamber gameobject
        /// </summary>
        ...
        public void NextChamber()
        {
            ...
            var flashMeshes = muzzleFlash.GetComponentsInChildren<MeshRenderer>(true);
            foreach (var mesh in flashMeshes)
            {
                mesh.enabled = true;
            }
            Invoke(nameof(HideMuzzleFlash), .1f);
            muzzleFlashParticles?.Emit(3);
            _currentChamber = (_currentChamber + 1) % 5;
            RotateChamber(_currentChamber - 1, _currentChamber, 0.1f);
        }
